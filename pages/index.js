    ...
    <>
      <section className={classnames(styles.section)}>
        <div className={styles["section-inner"]}>
          <h2 id="lore" className={styles.heading}>
            Lore
          </h2>

          <p>
            Since the beginning of time, the XIN Dragons have kept peace and
            order across the world. They protected the sky and the sea,
            controlled the seasons and protected mother nature. They lived in
            harmony with humans who worshipped them.
          </p>

          <p>
            Not so long ago came a big storm, and with it the dark forces
            prevailed. Some of the dragons became greedy, and their evil caused
            chaos around the world. In their pursuit to become immortal
            Superdragons, the detractors captured all the Baby Dragons and
            demanded XIN tokens from their parents to release them. The precious
            Xin tokens are the only way to unlock the prized Dragonballs, which
            must be collected and combined with a parent dragon to ultimately
            become a never-before-seen Superdragon!
          </p>

          <p>
            The whole world is now mobilized to help the righteous XIN Dragons
            bring the Baby Dragons back and restore peace, order and harmony.
          </p>

          <p>
            An epic battle is in sight... Are you going to be the one to pave
            the way... Which path will you choose?
          </p>
      ...
      <section className={classnames(styles.section, styles.dragons)}>
        <div className={styles["section-inner"]}>
          <h2 id="dragons" className={styles.heading}>
            Dragons
          </h2>
            ...
            {cards.map((card, index) => (
              <div key={index} className={styles.card}>
                <Image
                  ...
                  height={165}
                />
                <h3 className={classnames(styles.heading, styles[card.color])}>
                  {card.title}
                </h3>
                <p>{card.text}</p>
              </div>
      ...
      <section className={classnames(styles.section, styles.roadmap)}>
        <h2 id="roadmap" className={styles.heading}>
          The Evolved Roadmap
        </h2>
        <br />
        <div className={styles["roadmap-inner"]}>
          {phases.map((phase, index) => (
            <div
              key={index}
              className={classnames(styles.phase, styles[phase.align])}
              style={
                phase.marginTop ? { marginTop: `${phase.marginTop}px` } : {}
              }
            >
              <h3
                className={classnames(styles.heading, styles[phase.color])}
              >{`Phase ${index + 1} - ${phase.title}`}</h3>
              <div
              ...
              >
                <img
                  ...
                  alt="Cards"
                  style={
                    phase.offset ? { [phase.align]: `-${phase.offset}px` } : {}
                  }
                />
      ...
      <section
        id="the-chosen"
        className={classnames(styles.section, styles["the-chosen"])}
      >
        <div className={styles["section-inner"]}>
          <div className={classnames(styles.flex)}>
            <div className={styles["align-right"]}>
              ...
              <p className={styles.uppercase}>
                One from each ancient dragon family... and one mysterious
                overlord who reigns supreme over the entire weyr, from the
                deepest depths of the darkest corner of the universe...
              </p>
              <p>
                Each Chosen rules over its{" "}
                <span className={styles.lighter}>ENTIRE</span> clan, and demands
                that a <span className={styles.lighter}>TAX</span> is paid, in{" "}
                <span className={styles.lighter}>SOL</span>, every time one of
                its loyal subjects is traded in common barter (or via Magic Eden
                marketplace).
              </p>

              <p>
                That tax will be paid at a rate of{" "}
                <span className={styles.lighter}>8%</span> of the total
                royalties received.
              </p>

              <p>
                And for the one true overlord, a smaller tax must be paid for{" "}
                <span className={styles.lighter}>EVERY</span> dragon traded.
                This tax is set at <span className={styles.lighter}>3%</span> of{" "}
                <span className={styles.lighter}>ALL FUTURE ROYALTIES</span>{" "}
                from <span className={styles.lighter}>EVERY SINGLE SALE</span>
              </p>
            </div>
            <img
              src="/the-chosen.png"
